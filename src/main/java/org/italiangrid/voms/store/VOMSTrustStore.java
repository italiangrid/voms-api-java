...
public interface VOMSTrustStore {
   ...
   *
   * @return the collection of VOMS Attribute Authority certificates held in this {@link
   *     VOMSTrustStore}, an empty list if no certificate was found.
   */
  public List<X509Certificate> getLocalAACertificates();

  /**
   * Returns the VOMS Attribute Authority certificate held in this {@link VOMSTrustStore} whose
   * subject matches the subject passed as argument.
   *
   * @param aaCertSubject a certificate subject
   * @return the VOMS AA {@link X509Certificate} that matches the subject passed as argument or null
   *     if no matching certificate is found in this store
   */
  public X509Certificate getAACertificateBySubject(X500Principal aaCertSubject);

  /**
   * Returns the LSC information held in this {@link VOMSTrustStore} for the vo and hostname passed
   * as arguments.
   *
  ...
  /**
   * Returns all the LSC information held in this {@link VOMSTrustStore}. The returned {@link Map}
   * is keyed by VO name.
   *
  ...
  /**
   * Sets a {@link VOMSTrustStoreStatusListener} that is notified of events related to this VOMS
   * trust store
   *
