 ...
 * user of this API.
 *
 * <p>Credentials are searched in the following places (in sequence):
 *
 * <ul>
 *   <li>If the <code>X509_USER_CERT</code> and <code>X509_USER_KEY</code> <b>environment
 *       variables</b> are set, their values are used to load the user credentials
 ...
 *   <li>If the <code>PKCS12_USER_CERT</code> <b>environment variable</b> is set, its value is used
 *       to load the user credentials.
 *   <li>If the <code>PKCS12_USER_CERT</code> <b>system property</b> is set, its value is used to
 *       load the user credentials.
 *   <li>The content of the <code>.globus</code> directory in the user's home is searched for a PEM
 *       certificate (in the <code>usercert.pem</code> and <code>userkey.pem</code> files).
 *   <li>The content of the .globus directory in the user's home is searched for a PKC12 certificate
 *       (in the <code>usercert.p12</code> file).
 * </ul>
 */
