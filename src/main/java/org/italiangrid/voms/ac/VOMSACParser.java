...
public interface VOMSACParser {
  ...
  /**
   * Looks for and parses VOMS attributes in the certificate chain passed as argument (which is
   * assumed to be already validated).
   *
   * @param validatedChain a validated X.509 certificate chain
   * @return a possibly empty list of {@link VOMSAttribute} objects providing access to the parsed
   *     VOMS attributes
   */
