...
/**
 * This interface extends the {@link VOMSACParser} interface and provides methods to perform
 * validation on the VOMS Attribute Certificates parsed from a given certificate chain.
 *
...
public interface VOMSACValidator extends VOMSACParser {
  ...
  /**
   * Parses and validates the VOMS attributes found in the certificate chain passed as argument
   * (which is assumed to be validated already).
   *
   * <p>This method returns the possibly empty list of the validated attributes.
   *
   * <p>This method should be used in conjunction with the registration of a {@link
   * ValidationResultListener} to get details about validation error and warning messages.
   *
   * <p>Use the {@link #validateWithResult(X509Certificate[])} method in case you don't want to rely
   ...
   * (which is assumed to be validated already).
   *
   * <p>This method returns a possibly empty list of {@link VOMSValidationResult} objects which
   * describe the outcome of the validation for each VOMS AC found in the certificate chain.
   *
   * <p>This method is useful when you want to use a single call to get all details about validation
  ...
  /**
   * Sets a listener that will received validation-related events for this {@link VOMSACValidator}.
   *
   * @param listener the listener that will receive validation-related events.
   */
